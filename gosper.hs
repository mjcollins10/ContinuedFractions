...
-- find min,max of homomorphic matrix (axy + bx +cy + d)/(exy + fx + gy + h)
-- as x,y range from zero to infinity
...
allZero curMatrix = maximum (map maximum curMatrix) == 0 && minimum (map minimum curMatrix) == 0 
