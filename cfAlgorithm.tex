...
Gosper (in an appendix to the famous HAKMEM\footnote{\href{https://dspace.mit.edu/handle/1721.1/6086}{https://dspace.mit.edu/handle/1721.1/6086}} report) 
developed algorithms for adding, subtracting, multiplying, or dividing two CFs, and for solving quadratics with CF coefficients, getting a CF as the result\footnote{\href{https://perl.plover.com/classes/cftalk/INFO/gosper.html}{https://perl.plover.com/classes/cftalk/INFO/gosper.html}};
...
Our approach to the problem\footnote{For an entirely different approach see David Lester, ``Effective Continued Fractions", in \emph{Proceedings of 15th IEEE Symposium on Computer Arithmetic}, ARITH-15 2001, pp. 163-170.}
...
0 & 11 \\
1 & 0
\end{smallmatrix}\right)$. Of course it is easy to observe in this case that $M(3)>3$ and
$M(4)<4$, so the first term is $y_0=3$; but we must find a better algorithm than linear search through
$M(1), M(2), M(3)\cdots$. Such an algorithm exists because any homographic function of the form
 \[
 \begin{pmatrix}
p & q \\
r & -p
...
given any ``initial guess" $\hat{y}$, the fixed point is between $\hat{y}$ and $M(\hat{y})$. So we can
use binary search, starting perhaps from $\hat{y}=1$, to find the desired term in logarithmic time.
When $\hat{y} = \lfloor M(\hat{y}) \rfloor$ or $\hat{y} = \lceil M(\hat{y}) \rceil$, this common value is the next term of $y$. 
...
in other words, until the floor of the fixpoint is the same when $x=1$ and $x=\infty$. This floor will be the next term of $y$.
To find the next term, we first use the same binary search as before to get $\hat{y}$, the floor of the fixed point of of $M_1$; 
then we check whether $\hat{y}$ equals $\lfloor M_{\infty}(\hat{y}) \rfloor$ or $\lceil M_{\infty}(\hat{y}) \rceil$. If so, we ingest
...
\begin{figure}
\begin{algorithmic}\label{fig:sqrtCF}
...
\WHILE {$M \neq \infty$}
    \STATE{}\COMMENT{read input}
     ...
     \STATE{}\COMMENT{produce output if we can}
     \STATE{$\hat{y} \gets$ floor of fixed point of $M_{x_{\ell}}$} \COMMENT{found with binary search}
     \IF{floor of fixed point of $M_{x_h}$ equals $\hat{y}$}
...
The algorithm is not much different for $\mathbf{x}$ represented as a list of intervals. When we read an ambiguous interval
