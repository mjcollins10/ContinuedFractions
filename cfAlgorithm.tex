...
The key idea is that $\alpha$ is the fixed point of a homographic function, i.e. the solution to
\[
y = \frac{\alpha}{y}\ .
\]
...
0 & 11 \\
1 & 0
\end{smallmatrix}\right)$. Of course it is easy to observe in this simple case that $M(3)>3$ and
$M(4)<4$, so the first term is 3; but we must find a better algorithm than linear search through
$M(0), M(1), M(2)\cdots$. Such an algorithm exists because any homographic function of the form
 \[
 \begin{pmatrix}
p & q \\
r & -p
...
given any ``initial guess" $\hat{y}$, the fixed point is between $\hat{y}$ and $M(\hat{y})$. Thus we can
use binary search, starting perhaps from $\hat{y}=1$, to find the desired term in logarithmic time. When $\hat{y} = \lfloor M(\hat{y}) \rfloor$, this is the next term. 
The self-inverse property is preserved by ingesting and then producing the same term:
\[
\mbox{produce}\left(a, \mbox{ingest}\left(a,   \left(\begin{smallmatrix}
p & q \\
r & -p
...
operates on, and returns, our internal representation of a CF as a list of intervals.
...
1 & 0 
\end{smallmatrix}
\right)$ with constant $x$ and variable $y$. More generally let $M_{\mathbf{x}}$ denote the (homographic) matrix with $x$ fixed at $\mathbf{x}$,i.e. if $M=\abcd$ then
\[
...
We must ingest terms of $\mathbf{x}$ until we reach a stage at which the floor of the fixed point does not depend on $x$; in other words, until the floor of the fixpoint is the same when $x=1$ and $x=\infty$. This floor will be the next term of $y$.
To accomplish this, we use the same binary search as before to find $\hat{y}$, the floor of the fixed point of of $M_1$; 
then we check whether $\hat{y}$ equals $\lfloor M_{\infty}(\hat{y}) \rfloor$. If so, we ingest $\hat{y}$ and produce $\hat{y}$; if not, we ingest another term of $\mathbf{x}$ and continue.

This works because the matrices $M_x$ will always have the self-inverse property. This is clearly the case with the initial $M=\left(
\begin{smallmatrix} 
0 & 1 & 0 & 0\\ 
0 & 0 & 1 & 0
\end{smallmatrix}
\right)$, and a similar computation to shows that the property is preserved when we ingest and produce a term.

%As an example, we compute the fourth root of 2, i.e. the square root of $[1,(2)]$. 
