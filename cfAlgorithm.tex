...
If the denominator can be zero, $\rho\left(\abcd, [x_{\ell}, x_u),  [y_{\ell}, y_u)\right) = [1,\infty)$
...
If $|x| \leq \frac{\pi}{2}$, then $w < \frac{5}{2}$, and (\ref{eq:cosIter}) is an alternating series bounded between $1 - \frac{5}{2n(n+1)}$ and 1; the CF expansion of $c_n(w)$ will then start with zero, followed by the interval $(1, \frac{2n(n+1)}{2n(n+1)-5})$ and the rest of $\frac{1}{M_{n,w}(c_{n+1}(w))}$. When $n \geq 2$ this interval implies that the next CF term is $1$, so we can in fact append $1$ followed by the CF representation of
\[
